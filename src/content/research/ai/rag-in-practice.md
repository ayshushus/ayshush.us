...
RAG looks easy: fetch relevant text and answer grounded in it. Quality actually lives or dies in the retrieval pipeline. This note covers the stages and the ways each one fails.
...
Two embedding passes, one offline (documents) and one online (the query), meet in a vector index. Retrieval gives candidates that a reranker sharpens, and the LLM answers using only what survives.
...
- **No-answer cases.** If nothing relevant exists, retrieval still returns its top-k, and now you've handed the model irrelevant text it may treat as authoritative. Add a relevance threshold and let the model say "not found."
...
- Hybrid search + rerank (retrieve wide, narrow precisely) is the highest-impact default.
